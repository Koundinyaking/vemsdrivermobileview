...
-- OutTime Varchar(255));
--   
-- DROP TABLE IF EXISTS cabAllocationTable;

-- CREATE TABLE IF NOT EXISTS cabAllocationTable(
-- allocationId Varchar(255) primary key,
-- bookingId Varchar(255),
-- VehicleId Varchar(255),
-- EmployeeId Varchar(255), -- store multiple Employees in an array
-- priorityOrDer int,
-- allocateDDate Varchar(255));

-- DROP TABLE IF EXISTS CabAllocationML;

-- CREATE TABLE CabAllocationML(
-- AllocationId Varchar(255) primary key,
-- BookingId Varchar(255),
-- EmployeeId Varchar(255),
-- Latitude Varchar(255),
...
CREATE TABLE IF NOT EXISTS CabBookingTable (
    ...
    EscortNeeded BOOLEAN DEFAULT NULL,
...
DROP TABLE IF EXISTS TripManagement;

CREATE TABLE IF NOT EXISTS TripManagement(
...
-- ALTER TABLE CabBookingTable
-- ADD CONSTRAINT fk_CabBookingTable_EscortId
-- FOREIGN KEY (EscortId) REFERENCES EscortDetails(EscortId)
-- ON DELETE SET NULL
-- ON UPDATE CASCADE;

ALTER TABLE CabAllocationML
ADD CONSTRAINT fk_CabAllocationML_BookingId
FOREIGN KEY (BookingId) REFERENCES CabBookingTable(BookingId)
ON DELETE CASCADE
ON UPDATE CASCADE,
ADD CONSTRAINT fk_CabAllocationML_VehicleId
foreign key (VehicleId) REFERENCES VehicleDetails(VehicleId)
ON DELETE CASCADE
ON UPDATE CASCADE,
ADD CONSTRAINT fk_CabAllocationML_EmployeeeId
foreign key (EmployeeId) REFERENCES EmployeeDetails(EmployeeId)
ON DELETE CASCADE
ON UPDATE CASCADE;
...
ALTER TABLE TripManagement
ADD CONSTRAINT fk_TripManagement_BookingId
FOREIGN KEY (BookingId) REFERENCES CabBookingTable(BookingId)
ON DELETE CASCADE
ON UPDATE CASCADE,
ADD CONSTRAINT fk_TripManagement_EmployeeId 
FOREIGN KEY (EmployeeId) REFERENCES EmployeeDetails(EmployeeId)
ON DELETE CASCADE
ON UPDATE CASCADE,
ADD CONSTRAINT fk_TripManagement_DriverId
FOREIGN KEY (DriverId) REFERENCES DriverDetails(DriverId)
ON DELETE CASCADE
ON UPDATE CASCADE,
ADD CONSTRAINT fk_TripManagement_VehicleId
FOREIGN KEY (VehicleId) REFERENCES VehicleDetails(VehicleId)
ON DELETE CASCADE
ON UPDATE CASCADE;
...
DROP TRIGGER IF EXISTS autoGen_bookingId;
  ...
  -- SET NEW.BookingTime = Date_format(CURRENT_TIMESTAMP(), '%d/%m/%Y-%H:%i:%s');
  END;
  //
  
  DELIMITER ;

DROP TRIGGER IF EXISTS autoGen_allocationId;

  DELIMITER //
  CREATE TRIGGER autoGen_allocationId
  BEFORE INSERT ON CabAllocationML
  FOR EACH ROW 
  BEGIN
  DECLARE max_Id int;
  SELECT coalesce(max(cast(substring(AllocationId, 4)AS UNSIGNED)),0) into max_Id FROM CabAllocationML;
  SET NEW.AllocationId = CONCAT('AID', LPAD(max_Id + 1,3,0));
  END;
  //
...
DROP TRIGGER IF EXISTS autoGent_tripManagementId;

  DELIMITER //
  CREATE TRIGGER autoGen_tripManagement
  BEFORE INSERT ON TripManagement
  FOR EACH ROW
  BEGIN
  DECLARE max_Id int;
  SELECT coalesce(max(cast(SUBSTRING(TripId, 5) AS UNSIGNED)), 0) INTO max_Id FROM TripManagement;
...
DROP TRIGGER IF EXISTS update_VendorId_Driver;	-- To get VenDorId baseD on VenDorName in DriVer table --  
...
DROP TRIGGER IF EXISTS checkDriver; -- To check if the number of DriVers exceeDs the number of Vehicles --  
...
DROP TRIGGER IF EXISTS DriverLeaVeCount; -- To count the number of leaVes of DriVers --  
...
DROP TRIGGER IF EXISTS InsertTripManagement;

  DELIMITER //
  CREATE TRIGGER InsertTripManagement
  AFTER INSERT ON CabAllocationML
  FOR EACH ROW
  BEGIN
  DECLARE DId Varchar(255);
  DECLARE LogIn Varchar(255);
  DECLARE LogOut Varchar(255);
  SELECT DriverId INTO DId FROM DriverDetails WHERE VehicleId = NEW.VehicleId;
  SELECT LoginTime, LogoutTime INTO LogIn, LogOut FROM CabBookingTable WHERE BookingId = NEW.BookingId;
  INSERT INTO TripManagement(EmployeeId, DriverId, VehicleId, BookingId, StartTime, EndTime, TripDate)
  VALUES (NEW.EmployeeId, DId, NEW.VehicleId, NEW.BookingId, LogIn, LogOut, Date_format(current_Date(), '%d/%m/%Y'));
  END;
  //
...
BEGIN
    ...
    SET NEW.EmployeeGender = (SELECT EmployeeGender FROM EmployeeDetails WHERE EmployeeId = NEW.EmployeeId);
    SET NEW.EmployeeAddress = (SELECT EmployeeAddress FROM EmployeeDetails WHERE EmployeeId = NEW.EmployeeId);
    SET NEW.EmployeeCity = (SELECT EmployeeCity FROM EmployeeDetails WHERE EmployeeId = NEW.EmployeeId);
    SET NEW.Latitude = (SELECT Latitude FROM EmployeeDetails WHERE EmployeeId = NEW.EmployeeId);
    SET NEW.Longitude = (SELECT Longitude FROM EmployeeDetails WHERE EmployeeId = NEW.EmployeeId);
END;
//
DELIMITER ;
...
DROP TRIGGER IF EXISTS BookToAllocate;

DELIMITER //

CREATE TRIGGER BookToAllocate
AFTER INSERT ON cabbookingtable
FOR EACH ROW
BEGIN
  UPDATE caballocationml
...
DROP TRIGGER IF EXISTS ML_to_allocation;

DELIMITER //

CREATE TRIGGER ML_to_allocation
AFTER INSERT ON caballocationml
FOR EACH ROW
BEGIN
  INSERT INTO caballocationtable (AllocationId, BookingId, VehicleId, EmployeeId, Latitude, Longitude, PriorityOrder)
  VALUES (NEW.AllocationId, NEW.BookingId, NEW.VehicleId, NEW.EmployeeId, NEW.Latitude, NEW.Longitude, NEW.PriorityOrder);
END //
...
CREATE TRIGGER before_insert_cab_booking
BEFORE INSERT ON CabBookingTable
FOR EACH ROW
BEGIN
  IF NEW.EmployeeGender = 'Male' THEN
    SET NEW.EscortNeeded = NULL;
    SET NEW.EscortId = NULL;
  END IF;
END //
